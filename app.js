/* ===== Allowance Tracker (multi-child + admin) ===== */
(() => {
  ...
  const migrate = (parsed) => {
    ...
    if (!Array.isArray(parsed.children) || parsed.children.length === 0) {
      // Legacy single-child structure → wrap in a default kid
      const child = {
        ...defaultChild(),
  ...
  const logActivity = (action, details = {}) => {
    ...
    state.activity.push({ id: uuid(), ts: Date.now(), actor, actorRole: role, action, ...details });
  ...
  const runningBalanceForBuckets = (buckets, cid) => {
    ...
    return buckets.map(b => {
      ...
      for (const t of sorted) {
        if (t.ts <= b.end) bal += t.type === "earn" ? t.amount : -t.amount;
        else break;
      }
  ...
  const renderFamily = () => { renderChildren(); renderActivity(); };
  ...
  const approveAllFor = (cid) => {
    let n = 0;
    pendingFor(cid).forEach(t => { t.status = "approved"; n++; });
    if (n) {
      const c = state.children.find(x => x.id === cid);
      logActivity("approved-all", { childId: cid, summary: `${n} items${c ? " · " + c.name : ""}` });
    }
    save(); renderAll();
  };
  ...
  const rejectAllFor = (cid) => {
    let n = 0;
    pendingFor(cid).forEach(t => { t.status = "rejected"; n++; });
    if (n) {
      const c = state.children.find(x => x.id === cid);
      logActivity("rejected-all", { childId: cid, summary: `${n} items${c ? " · " + c.name : ""}` });
    }
    save(); renderAll();
  };
  ...
  document.addEventListener("click", (e) => {
    ...
    if (action === "payday" && c) doPaydayFor(c.id);
      ...
      if (confirm("Clear the activity log?")) { state.activity = []; save(); renderAll(); }
    }
  });
  ...
  document.addEventListener("click", (e) => {
    ...
    if (ek) openKidEdit(ek.dataset.editKid);
      ...
      state.auth.activeChildId = vk.dataset.viewKid; save();
  ...
  $("#importFile").addEventListener("change", async (e) => {
    ...
    try {
      ...
      if (data?.child && Array.isArray(data.txns)) {
        ...
        state = migrate(data); save(); renderAll();
  ...
  /* ===========================================================
     SETTINGS MODAL (parent PIN only)
     =========================================================== */
  ...
  const openKidEdit = (id) => {
    ...
    $("#kidName").value = c.name;
    $("#kidEmoji").value = c.emoji;
    $("#kidWeekly").value = c.weekly;
    $("#kidPayday").value = c.payday;
    $("#kidGoalName").value = c.goal.name;
    $("#kidGoalTotal").value = c.goal.total;
    $("#kidPin").value = "";
  ...
  const openChildPicker = () => {
    ...
    state.children.forEach(c => {
      ...
      btn.addEventListener("click", () => {
        ...
        save(); pickerModal.close(); renderAll();
      });
  ...
  const advanceSetup = async () => {
    if (setupStep === "new") {
      ...
      if (setupBuffer === setupPending) {
        ...
        ensureActiveChild();
        logActivity("first-time setup", { summary: "Parent account created" });
  ...
  const tryLogin = async () => {
    ...
    if (hashed === expected) {
      if (loginTarget === "parent") {
        ...
        state.auth.activeRole = "child";
        state.auth.activeChildId = loginTarget;
  ...
  /* ===========================================================
     ...
     =========================================================== */
  const bootAuth = () => {
    if (!state.auth.parentPin) startSetup();
    else if (!state.auth.activeRole) startLogin();
